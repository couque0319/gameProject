...
(function () {
  ...
  function onBuyButtonClick(e) {
    ...
    if (typeof window.Game?.player?.coins === 'undefined' || window.Game.player.coins < cost) {
      showToast('코인이 부족합니다!');
      return;
    }
    ...
    if (typeof window.PlayerUpgrade !== 'undefined' && typeof window.PlayerUpgrade.buy === 'function') {
      ...
      // 코인 차감
      window.Game.player.coins -= cost;
      // 게임 진행 상태 저장 (코인)
      if (typeof window.Game?.saveGameProgress === 'function') {
        window.Game.saveGameProgress();
      }

      // 버튼 UI 업데이트
      // e.currentTarget.disabled = true; // updateShopStatusUI에서 처리
      // e.currentTarget.classList.add('bought'); // updateShopStatusUI에서 처리
  ...
  function updateShopStatusUI(snapshot) {
    ...
    const currentCoins = window.Game?.player?.coins || 0;
    ...
    shopButtons.forEach(btn => {
      ...
      const cost = COSTS[key] || Infinity;
      let isDisabled = false;
      let isPurchased = false;

      // 이미 구매한 아이템 처리
      if (key === '1' || key === '2' || key === '3' || key === '4') {
        isPurchased = shopState.purchasedOnce[key];
      } else if (key === '5') { // 연사력
        isPurchased = shopState.tier.a >= 3; // 최대 레벨
      } else if (key === '6') { // 데미지
        isPurchased = shopState.tier.s >= 3; // 최대 레벨
      } else if (key === '7') { // 체력
        isPurchased = shopState.tier.d >= 3; // 최대 레벨
      }

      if (isPurchased) {
        btn.disabled = true;
        btn.classList.add('bought');
        btn.textContent = btn.textContent.replace('구매', '최대'); // 텍스트 변경
      } else {
        // 미사일 관련 아이템 (3, 4)은 기능 미구현으로 항상 비활성화
        if (key === '3' || key === '4') {
          isDisabled = true;
          btn.textContent = btn.textContent.split(' (')[0] + ' (미구현)';
        } else {
          // 코인 부족 여부 확인
          isDisabled = currentCoins < cost;
          btn.textContent = btn.textContent.split(' (')[0] + ` (${cost} 코인)`; // 비용 표시
        }
        btn.disabled = isDisabled;
        btn.classList.toggle('not-enough-coins', isDisabled);
      }
    });
  }
